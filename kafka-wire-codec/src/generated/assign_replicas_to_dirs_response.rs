...
impl DirectoryData {
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        let mut msg = DirectoryData::default();
        {
            msg.id = get_uuid(buf)?;
        }
        {
...
impl AssignReplicasToDirsResponse {
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        ...
        let mut msg = AssignReplicasToDirsResponse::default();
        {
        ...
        }
        {
            ...
            let count = len_opt.ok_or(DecodeError::NullForNonNullable)?;
            { let mut items = Vec::with_capacity(count.min(buf.len()));
                for _ in 0..count { items.push(DirectoryData::decode(version, buf)?); }
            msg.directories = items; }
        }
        msg.tagged_fields = get_tagged_fields(buf)?;
        Ok(msg)
    }
...
impl crate::Encodable for AssignReplicasToDirsResponse {
...
impl crate::EncodableZeroCopy for AssignReplicasToDirsResponse {
