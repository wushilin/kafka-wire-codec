    ...
    pub fn encoded_size(&self, version: i16) -> usize {
        ...
        let mut size = 0usize;
        {
            { let arr = &self.groups_names;
                ...
                for item in arr {
                    size += if version >= 2 { compact_string_size(item) } else { string_size(item) };
                }
            }
        }
    ...
    pub fn encode<B: WireBuf>(&self, version: i16, buf: &mut B) {
            ...
            "unsupported version {} for api key {}", version, Self::API_KEY);
        {
            { let arr = &self.groups_names;
                ...
                for item in arr { if version >= 2 { put_compact_string(buf, item); } else { put_string(buf, item); } }
            }
        }
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        ...
        let mut msg = DeleteGroupsRequest::default();
        {
            ...
            { let mut items = Vec::with_capacity(count.min(buf.len()));
                for _ in 0..count { items.push((if version >= 2 { get_compact_string(buf) } else { get_string(buf) }).and_then(|o| o.ok_or(DecodeError::NullForNonNullable))?); }
