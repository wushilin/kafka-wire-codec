...
impl Default for CreateDelegationTokenRequest {
    fn default() -> Self {
        Self {
            owner_principal_type: Some(Bytes::new()),
            owner_principal_name: Some(Bytes::new()),
            renewers: Vec::new(),
            max_lifetime_ms: 0,
...
impl CreateDelegationTokenRequest {
    ...
    pub fn encoded_size(&self, version: i16) -> usize {
        ...
        let mut size = 0usize;
        if version >= 3 {
            size += if version >= 3 { if version >= 2 { compact_nullable_string_size(self.owner_principal_type.as_deref()) } else { nullable_string_size(self.owner_principal_type.as_deref()) } } else { let v = self.owner_principal_type.as_deref().expect("field owner_principal_type is None but not nullable at this version"); if version >= 2 { compact_string_size(v) } else { string_size(v) } };
        }
        if version >= 3 {
            size += if version >= 3 { if version >= 2 { compact_nullable_string_size(self.owner_principal_name.as_deref()) } else { nullable_string_size(self.owner_principal_name.as_deref()) } } else { let v = self.owner_principal_name.as_deref().expect("field owner_principal_name is None but not nullable at this version"); if version >= 2 { compact_string_size(v) } else { string_size(v) } };
        }
        {
    ...
    pub fn encode<B: WireBuf>(&self, version: i16, buf: &mut B) {
        assert!((Self::VALID_MIN_VERSION..=Self::VALID_MAX_VERSION).contains(&version),
            "unsupported version {} for api key {}", version, Self::API_KEY);
        if version >= 3 {
            if version >= 3 { if version >= 2 { put_compact_nullable_string(buf, self.owner_principal_type.as_deref()) } else { put_nullable_string(buf, self.owner_principal_type.as_deref()) } } else { let v = self.owner_principal_type.as_deref().expect("field owner_principal_type is None but not nullable at this version"); if version >= 2 { put_compact_string(buf, v) } else { put_string(buf, v) } };
        }
        if version >= 3 {
            if version >= 3 { if version >= 2 { put_compact_nullable_string(buf, self.owner_principal_name.as_deref()) } else { put_nullable_string(buf, self.owner_principal_name.as_deref()) } } else { let v = self.owner_principal_name.as_deref().expect("field owner_principal_name is None but not nullable at this version"); if version >= 2 { put_compact_string(buf, v) } else { put_string(buf, v) } };
        }
        {
            { let arr = &self.renewers;
