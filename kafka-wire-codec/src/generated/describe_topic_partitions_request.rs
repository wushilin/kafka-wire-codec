...
impl DescribeTopicPartitionsRequest {
    ...
    pub fn encoded_size(&self, version: i16) -> usize {
        ...
        {
            size += 1 + self.cursor.as_ref().map_or(0, |v| v.encoded_size(version));
        }
        size += tagged_fields_size(&self.tagged_fields);
        size
    }
    ...
    pub fn encode<B: WireBuf>(&self, version: i16, buf: &mut B) {
        ...
        {
            match &self.cursor { Some(v) => { put_i8(buf, 1); v.encode(version, buf); }, None => put_i8(buf, -1) };
        }
        put_tagged_fields(buf, &self.tagged_fields);
    }
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        ...
        let mut msg = DescribeTopicPartitionsRequest::default();
        {
            let len_opt = { let n = get_uvarint32(buf)?; if n == 0 { None } else { Some((n - 1) as usize) } };
            let count = len_opt.ok_or(DecodeError::NullForNonNullable)?;
            { let mut items = Vec::with_capacity(count.min(buf.len()));
                for _ in 0..count { items.push(TopicRequest::decode(version, buf)?); }
            msg.topics = items; }
        }
        {
            msg.response_partition_limit = get_i32(buf)?;
        }
        {
            msg.cursor = if get_i8(buf)? < 0 { None } else { Some(Cursor::decode(version, buf)?) };
        }
        msg.tagged_fields = get_tagged_fields(buf)?;
        Ok(msg)
    }
...
impl crate::Encodable for DescribeTopicPartitionsRequest {
...
impl crate::EncodableZeroCopy for DescribeTopicPartitionsRequest {
