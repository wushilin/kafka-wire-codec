...
impl AlterableConfig {
    pub fn encoded_size(&self, version: i16) -> usize {
        ...
        {
            size += if version >= 2 { compact_nullable_string_size(self.value.as_deref()) } else { nullable_string_size(self.value.as_deref()) };
        }
        if version >= 2 { size += tagged_fields_size(&self.tagged_fields); }
        size
    }
    ...
    pub fn encode<B: WireBuf>(&self, version: i16, buf: &mut B) {
        ...
        {
            if version >= 2 { put_compact_nullable_string(buf, self.value.as_deref()) } else { put_nullable_string(buf, self.value.as_deref()) };
        }
        if version >= 2 { put_tagged_fields(buf, &self.tagged_fields); }
    }
