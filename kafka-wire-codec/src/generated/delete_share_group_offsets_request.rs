    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        ...
        }
        {
            ...
            { let mut items = Vec::with_capacity(count.min(buf.len()));
                for _ in 0..count { items.push(DeleteShareGroupOffsetsRequestTopic::decode(version, buf)?); }
            msg.topics = items; }
        }
        msg.tagged_fields = get_tagged_fields(buf)?;
        Ok(msg)
...
impl crate::Encodable for DeleteShareGroupOffsetsRequest {
...
impl crate::EncodableZeroCopy for DeleteShareGroupOffsetsRequest {
