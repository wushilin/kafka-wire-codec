...
impl ReadStateSummaryResult {
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        let mut msg = ReadStateSummaryResult::default();
        {
...
impl ReadShareGroupStateSummaryResponse {
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        ...
        let mut msg = ReadShareGroupStateSummaryResponse::default();
        {
            let len_opt = { let n = get_uvarint32(buf)?; if n == 0 { None } else { Some((n - 1) as usize) } };
            let count = len_opt.ok_or(DecodeError::NullForNonNullable)?;
            { let mut items = Vec::with_capacity(count.min(buf.len()));
                for _ in 0..count { items.push(ReadStateSummaryResult::decode(version, buf)?); }
            msg.results = items; }
        }
        msg.tagged_fields = get_tagged_fields(buf)?;
        Ok(msg)
...
impl crate::Encodable for ReadShareGroupStateSummaryResponse {
...
impl crate::EncodableZeroCopy for ReadShareGroupStateSummaryResponse {
