...
/// Shell (chunked-payload) variant of [`FetchableTopicResponse`]: identical except records
/// payloads are `RecordsChunks` chunk chains instead of contiguous `Bytes`.
#[derive(Debug, Clone, PartialEq, Default)]
pub struct FetchableTopicResponseShell {
    /// The topic name.
    pub topic: TopicName,
    /// The unique topic ID.
    pub topic_id: Uuid,
...
impl FetchableTopicResponseShell {
    ...
    pub fn decode_chained(version: i16, ch: &mut ChunkChain) -> Result<Self, DecodeError> {
        let mut msg = FetchableTopicResponseShell::default();
        if version <= 12 {
            msg.topic = TopicName((if version >= 12 { ch_get_compact_string(ch)? } else { ch_get_string(ch)? }).ok_or(DecodeError::NullForNonNullable)?);
        }
        if version >= 13 {
            msg.topic_id = ch_get_uuid(ch)?;
        }
        {
            let len_opt = if version >= 12 { { let n = ch_get_uvarint32(ch)?; if n == 0 { None } else { Some((n - 1) as usize) } } } else { { let n = ch_get_i32(ch)?; if n < 0 { None } else { Some(n as usize) } } };
            let count = len_opt.ok_or(DecodeError::NullForNonNullable)?;
        ...
        if version >= 12 { msg.tagged_fields = ch_get_tagged_fields(ch)?; }
        Ok(msg)
    }
...
impl PartitionDataShell {
    ...
    pub fn decode_chained(version: i16, ch: &mut ChunkChain) -> Result<Self, DecodeError> {
        ...
        {
            msg.high_watermark = ch_get_i64(ch)?;
        }
        if version >= 4 {
            msg.last_stable_offset = ch_get_i64(ch)?;
        }
        if version >= 5 {
            msg.log_start_offset = ch_get_i64(ch)?;
        }
        if version >= 4 {
            let len_opt = if version >= 12 { { let n = ch_get_uvarint32(ch)?; if n == 0 { None } else { Some((n - 1) as usize) } } } else { { let n = ch_get_i32(ch)?; if n < 0 { None } else { Some(n as usize) } } };
            msg.aborted_transactions = match len_opt {
                Some(count) => {
                let mut items = Vec::with_capacity(count.min(ch.remaining()));
                for _ in 0..count { items.push(AbortedTransaction::decode_chained(version, ch)?); }
                Some(items)
                }
                None => { if version >= 4 { None } else { return Err(DecodeError::NullForNonNullable); } }
            };
        ...
        if version >= 11 {
            msg.preferred_read_replica = BrokerId(ch_get_i32(ch)?);
        }
        {
            msg.records = if version >= 12 { ch_get_compact_records(ch)? } else { ch_get_records(ch)? };
        }
        if version >= 12 { {
    ...
    pub fn encoded_size(&self, version: i16) -> usize {
        ...
        {
            size += if version >= 12 { compact_nullable_records_chunks_size(self.records.as_ref()) } else { nullable_records_chunks_size(self.records.as_ref()) };
        }
        if version >= 12 { {
            let mut num_tagged = self.tagged_fields.len();
            let mut known_tagged_size = 0usize;
    ...
    pub fn encode<B: WireBuf>(&self, version: i16, buf: &mut B) {
        ...
        {
            if version >= 12 { put_compact_nullable_records_chunks_zc(buf, self.records.as_ref()) } else { put_nullable_records_chunks_zc(buf, self.records.as_ref()) };
        }
        if version >= 12 { {
            let mut num_tagged = self.tagged_fields.len();
            if version >= 12 && (self.diverging_epoch != EpochEndOffset::default()) { num_tagged += 1; }
...
impl AbortedTransaction {
    ...
    pub fn decode_chained(version: i16, ch: &mut ChunkChain) -> Result<Self, DecodeError> {
        let mut msg = AbortedTransaction::default();
        if version >= 4 {
            msg.producer_id = ProducerId(ch_get_i64(ch)?);
        }
        if version >= 4 {
...
/// Shell (chunked-payload) variant of [`FetchResponse`]: identical except records
/// payloads are `RecordsChunks` chunk chains instead of contiguous `Bytes`.
#[derive(Debug, Clone, PartialEq, Default)]
pub struct FetchResponseShell {
    /// The duration in milliseconds for which the request was throttled due to a quota violation, or zero if the request did not violate any quota.
    pub throttle_time_ms: i32,
    /// The top level response error code.
    pub error_code: i16,
    ...
    pub session_id: i32,
    /// The response topics.
    pub responses: Vec<FetchableTopicResponseShell>,
    /// Endpoints for all current-leaders enumerated in PartitionData, with errors NOT_LEADER_OR_FOLLOWER & FENCED_LEADER_EPOCH.
    pub node_endpoints: Vec<NodeEndpoint>,
    /// Unknown/raw tagged fields (flexible versions), ascending tag order.
    pub tagged_fields: Vec<(u32, Bytes)>,
...
impl FetchResponseShell {
    /// Decode from a chunk chain; records payloads come out as zero-copy
    /// chunk slices (see `frame::read_frame_supplied`).
    pub fn decode_chained(version: i16, ch: &mut ChunkChain) -> Result<Self, DecodeError> {
        if !(FetchResponse::VALID_MIN_VERSION..=FetchResponse::VALID_MAX_VERSION).contains(&version) {
            return Err(DecodeError::UnsupportedVersion { api_key: FetchResponse::API_KEY, version });
        }
        let mut msg = FetchResponseShell::default();
        if version >= 1 {
        ...
        if version >= 7 {
            msg.session_id = ch_get_i32(ch)?;
        }
        {
            let len_opt = if version >= 12 { { let n = ch_get_uvarint32(ch)?; if n == 0 { None } else { Some((n - 1) as usize) } } } else { { let n = ch_get_i32(ch)?; if n < 0 { None } else { Some(n as usize) } } };
            let count = len_opt.ok_or(DecodeError::NullForNonNullable)?;
            { let mut items = Vec::with_capacity(count.min(ch.remaining()));
                for _ in 0..count { items.push(FetchableTopicResponseShell::decode_chained(version, ch)?); }
            msg.responses = items; }
        }
        if version >= 12 { {
            let count = ch_get_uvarint32(ch)? as usize;
    ...
    pub fn encoded_size(&self, version: i16) -> Result<usize, EncodeError> {
        if !(FetchResponse::VALID_MIN_VERSION..=FetchResponse::VALID_MAX_VERSION).contains(&version) {
            return Err(EncodeError::UnsupportedVersion { api_key: FetchResponse::API_KEY, version });
        }
        let mut size = 0usize;
        if version >= 1 {
            size += 4;
