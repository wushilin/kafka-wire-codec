...
impl ReadStateData {
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        let mut msg = ReadStateData::default();
        {
...
impl ReadShareGroupStateRequest {
    ...
    pub fn decode(version: i16, buf: &mut Bytes) -> Result<Self, DecodeError> {
        ...
        let mut msg = ReadShareGroupStateRequest::default();
        {
        ...
        }
        {
            ...
            let count = len_opt.ok_or(DecodeError::NullForNonNullable)?;
            { let mut items = Vec::with_capacity(count.min(buf.len()));
                for _ in 0..count { items.push(ReadStateData::decode(version, buf)?); }
            msg.topics = items; }
        }
        msg.tagged_fields = get_tagged_fields(buf)?;
        Ok(msg)
...
impl crate::Encodable for ReadShareGroupStateRequest {
...
impl crate::EncodableZeroCopy for ReadShareGroupStateRequest {
